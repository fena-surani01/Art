...
def update_customArt_ejs():
    ...
    # Step 1: Replace form structure
    form_start_str = '''                    <form id="customArtForm" enctype="multipart/form-data" novalidate>
    ...
    content = content.replace(form_start_str, new_form_structure)

    # Step 2: Replace Submit button in Step 1 to a "Next" button, and add Step 2 & 3
    submit_str = '''                        <!-- Price Estimation & Submit -->
                        <div class="row align-items-center bg-light p-4 rounded-4">
                            <div class="col-md-6 mb-3 mb-md-0 text-center text-md-start">
                                <div class="text-muted fw-bold text-uppercase small letter-spacing-1">Estimated Price</div>
                        ...
                        <div id="step2" style="display: none;">
                            ...
                            <div class="row g-4 mb-4">
                                <div class="col-md-4">
                                    ...
                                    <input type="text" class="form-control form-control-lg" name="shipping_city" id="shipping_city" required>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">State (Required)</label>
                                    <input type="text" class="form-control form-control-lg" name="shipping_state" id="shipping_state" required>
                                </div>
                                <div class="col-md-4">
    ...
    content = content.replace(submit_str, new_submit_structure)

    # Replace JS script
    script_start = '''            // Pricing Logic
            const calculatePrice = () => {'''
            
    script_end = '''// Form Submission
            form.addEventListener('submit', async (e) => {'''

    pattern = re.compile(r'// Pricing Logic.*?(?=(// Form Submission))', re.DOTALL)
    ...
    # Now remove the old validation in the submit event since we do it step by step
    old_validation = '''                // --- Validation ---
                const img = document.getElementById('reference_image');
                if (!img.files || img.files.length === 0) {
                    return showToast('Please upload a reference image.', 'danger');
                }
                
                const size = document.getElementById('art_size').value;
                if (!size) {
                    return showToast('Please select an artwork size.', 'danger');
                }
                
                const qty = document.getElementById('qty').value;
                if (!qty || qty < 1) {
                    return showToast('Please enter a valid number of subjects.', 'danger');
                }
                
                const format = document.querySelector('input[name="format"]:checked');
                if (!format) {
                    return showToast('Please select an artwork style.', 'danger');
                }
                
                const desc = document.getElementById('description').value.trim();
                if (!desc) {
                    return showToast('Please provide special instructions.', 'danger');
                }
                // -------------------'''
